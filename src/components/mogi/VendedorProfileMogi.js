...
export default function VendedorProfileMogi({ user, onBack }) {
  ...
  const carregarEstatisticas = async () => {
    try {
      ...
      // Vendas do mês
      const { data: vendasMes } = await queryWithStoreMogi('vendas')
        .select('valor_final')
      ...
      // Vendas de hoje
      const { data: vendasHoje } = await queryWithStoreMogi('vendas')
        .select('valor_final')
        .eq('vendedor_nome', user.nome)
      ...
      const { data: itensVendidos } = await queryWithStoreMogi('itens_venda')
        .select(`
          produto_nome,
          quantidade,
          vendas_mogi!inner(vendedor_nome, data_venda)
        `)
        .eq('vendas_mogi.vendedor_nome', user.nome)
        .gte('vendas_mogi.data_venda', inicioMes.toISOString());
