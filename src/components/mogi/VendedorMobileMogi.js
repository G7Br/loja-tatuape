...
import QRScanner from './QRScannerMogi';
...
export default function VendedorMobileMogi({ user, onLogout }) {
  ...
  const carregarVendasStandby = async () => {
    const { data, error } = await queryWithStoreMogi('vendas_standby')
      .select('*')
      .order('created_at', { ascending: false });
    if (error) console.error('Erro ao carregar vendas standby:', error);
  ...
  const verificarClienteStandby = async (telefone) => {
    ...
    if (error && error.code !== 'PGRST116') console.error('Erro ao verificar cliente standby:', error);
    return data;
  };
  ...
  const cadastrarCliente = async (dadosCliente) => {
    ...
    const { error } = await queryWithStoreMogi('clientes')
      .upsert({
  ...
  const enviarParaStandby = async () => {
    ...
    try {
      ...
      // Salvar no banco
      const { error } = await queryWithStoreMogi('vendas_standby')
        .insert([{
      ...
      if (error) {
        console.error('Erro detalhado ao inserir standby:', error);
        throw error;
      }
  ...
  const editarVendaStandby = async (venda) => {
    try {
      ...
      // Remove do banco
      const { error: deleteError } = await queryWithStoreMogi('vendas_standby')
        .delete()
        .eq('id', venda.id);
      
      if (deleteError) console.error('Erro ao deletar venda standby:', deleteError);
  ...
  const finalizarVendaStandby = async (venda) => {
    ...
    try {
      const { data: vendaData, error: vendaError } = await queryWithStoreMogi('vendas')
        .insert([{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{display: 'flex', alignItems: 'center', gap: '10px'}}>
            ...
            />
            <div>
              <h1 style={{ fontSize: '1.2rem', fontWeight: '700', margin: 0 }}>VENDAS - MOGI</h1>
            ...
            <input
              ...
              onChange={async (e) => {
                ...
                if (e.target.value.length >= 11) {
                  const { data: cpfExistente } = await queryWithStoreMogi('clientes')
      ...
      {carrinho.length > 0 && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button onClick={enviarParaStandby} disabled={clienteEmStandby || cpfJaExiste || processandoVenda || !clienteTelefone.trim()} style={{
              flex: 1,
              padding: '12px 16px',
              background: (clienteEmStandby || cpfJaExiste || processandoVenda || !clienteTelefone.trim()) ? '#666' : '#f59e0b',
              color: '#ffffff',
              border: 'none',
              borderRadius: '8px',
              cursor: (clienteEmStandby || cpfJaExiste || processandoVenda || !clienteTelefone.trim()) ? 'not-allowed' : 'pointer',
              fontWeight: '600',
              fontSize: '0.9rem'
            }}>
              Standby
            </button>
            <button onClick={finalizarVenda} disabled={clienteEmStandby || processandoVenda || !clienteTelefone.trim()} style={{
              flex: 1,
              padding: '12px 16px',
              background: (clienteEmStandby || processandoVenda || !clienteTelefone.trim()) ? '#666' : '#10b981',
              color: '#ffffff',
              border: 'none',
              borderRadius: '8px',
              cursor: (clienteEmStandby || processandoVenda || !clienteTelefone.trim()) ? 'not-allowed' : 'pointer',
              fontWeight: '600',
              fontSize: '0.9rem'
            }}>
              Enviar ao Caixa
